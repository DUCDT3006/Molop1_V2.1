/* ====================================
   Molop1 V2.1 - Basic Vietnamese (with confirm button)
   ==================================== */
import { getLesson } from '../data/lessons.js';
import { getState, markComplete, showConfetti } from '../data/state.js';

export function renderBasicVietnamese(container, params) {
  ...
  const bv = lesson.basicVietnamese;
  ...
  function renderReading() {
      ...
      <div class="game-container">
        ...
        <div class="reading-card">
          ...
          <div class="reading-meta">
            <span>📝 ${bv.passage.wordCount} từ</span>
    ...
    container.querySelector('#done-reading').addEventListener('click', renderQuiz);
  }

  function renderQuiz() {
    const q = bv.questions[currentQ];
    selectedAnswer = null;
    container.innerHTML = `
      <div class="header-bar">
        <button class="back-btn" onclick="navigateTo('#daily/${week}/${day}')">←</button>
        <div class="title">📖 ${bv.title} - Câu hỏi</div>
        <div class="star-counter">⭐ ${state.stars}</div>
      </div>
      <div class="game-container">
        <div class="section-badge basic">📚 Cơ bản - Tiếng Việt</div>
        <div class="game-card">
          <div class="question-counter">Câu ${currentQ + 1} / ${bv.questions.length}</div>
    ...
    confirmBtn.addEventListener('click', () => {
      ...
      setTimeout(() => { currentQ++; if (currentQ < bv.questions.length) renderQuiz(); else showResults(); }, 1200);
    });
  }

  function showResults() {
    const pct = Math.round((correctCount / bv.questions.length) * 100);
    ...
    markComplete(week, day, 'basic-viet', rating);
    ...
    container.innerHTML = `
      <div class="reward-screen" style="position:relative"><div class="reward-content">
        ...
        <div class="score-display">${correctCount}/${bv.questions.length}</div>
        <div class="reward-stars">${stars}</div>
        <div class="reward-subtitle">Đúng ${pct}% câu hỏi đọc hiểu</div>
        <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
          <button class="btn btn-primary" onclick="navigateTo('#daily/${week}/${day}')">📋 Quay lại</button>
          ${pct < 100 ? `<button class="btn btn-secondary" onclick="navigateTo('#basic-viet/${week}/${day}')">🔄 Làm lại</button>` : ''}
        </div>
      </div></div>`;
  }
  renderReading();
